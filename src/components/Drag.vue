...
export default {
 ...
 directives: {
  move(el) {
    ...
    el.style.left = endx + 'rem'
    // console.log(el.parentNode.children[0])
    var width = $('.movebox').width() - $('.move').width()
     el.parentNode.children[0].style.width = endx + 'rem'
     el.parentNode.children[1].innerHTML = '拖动滑块验证'
    //临界值小于
    if (endx <= 0) {
    el.style.left = 0 + 'rem'
    el.parentNode.children[0].style.width = 0 + 'rem'
     //$('.movego').width(0)
    }
    //临界值大于
    // console.log(el.style.left)
    if (parseInt(el.style.left) >= width) {
    el.style.left = width + 'rem'
     el.parentNode.children[0].style.width = width + 'rem'
